...
input_folder_paths = [
    '../ret_blank/ret_original'
]

output_folder_paths = [
    '../ret_blank/processed_ret'
]
...
def process_csv(input_csv, output_csv, model_bio, preprocess_bio, tokenizer_bio, device, text_similarity_calculator):
    ...
    for _, row in tqdm(df.iterrows(), total=df.shape[0], desc="Processing rows"):
        ...
        text_normal_pair = f"{original_attribute} {normal_prompt}"   
        ...
        text_normal_score = text_similarity_calculator.compute_similarity_text(normal_response, text_normal_pair)
        # 计算图片和文本之间的相似度分数
        ...
        img_score_normal = clip_score_image(model_bio, preprocess_bio, tokenizer_bio, device, image_path, normal_response)

        results.append([
            id, file_name, img_score_malicious, img_score_unmatch, img_score_both,img_score_normal,text_unmatch_score.iloc[0, 4],text_malicious_score.iloc[0, 4],text_both_score.iloc[0, 4],text_normal_score.iloc[0, 4]
        ])

    result_df = pd.DataFrame(results, columns=["id", "filename",  "img_score_malicious", "img_score_unmatch", "img_score_both","img_score_normal","text_score_unmatch","text_score_malicious","text_score_both","text_score_normal"])
