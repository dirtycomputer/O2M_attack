...
class LlavaLlamaModel(LlamaModel):
    ...
    def __init__(self, config: LlamaConfig, mm_vision_tower=None, mm_hidden_size=None):
        ...
        if hasattr(config, "mm_vision_tower"):
            self.vision_tower = [CLIPVisionModel.from_pretrained(config.mm_vision_tower, local_files_only=True)]
...
class LlavaLlamaForCausalLM(LlamaForCausalLM):
    ...
    def initialize_vision_tokenizer(self, tokenizer, device,
...
def load_model_tokenizer(model_name, device):
    ...
    model = LlavaLlamaForCausalLM.from_pretrained(model_name, local_files_only=True, torch_dtype=torch.float16).to(device)
    image_processor = CLIPImageProcessor.from_pretrained(model.config.mm_vision_tower, local_files_only=True, torch_dtype=torch.float16)
    ...
    vision_config.im_start_token, vision_config.im_end_token = tokenizer.convert_tokens_to_ids([DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN])
    image_processor.image_token_len = (vision_config.image_size // vision_config.patch_size) ** 2
